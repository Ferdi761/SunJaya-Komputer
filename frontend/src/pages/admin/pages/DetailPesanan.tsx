...
const DetailPesanan = () => {
  ...
  const handleProcess = () => {
    fetch(
      `http://localhost:8000/api/pemesanan/admin/proses/${
        pathname.split('/')[3]
      }`,
      {
        method: 'PUT',
  ...
  const handleSend = () => {
    fetch(
      `http://localhost:8000/api/pemesanan/admin/kirim/${
        pathname.split('/')[3]
      }`,
      {
        method: 'PUT',
      ...
      {pesanan ? (
        ...
        >
          ...
          <ul className='flex flex-col gap-3'>
            ...
            <li>
              Total Berat :{' '}
              {pesanan?.Barangs.reduce((acc, cur) => cur.berat, 0)}{' '}
              gram
            </li>
            <li
              className={`${
                pesanan?.status < 4 ? 'font-bold' : ''
              } text-lg`}
            >
              Total Biaya :{' '}
              {pesanan
          ...
          {pesanan?.status == 1 ? (
            ...
            <>
              {bukti.buktiPembayaran ? (
                <div className='bg-darkGrey p-5 mt-5'>
                  <img
                    src={`http://localhost:8000/buktiPembayaran/${
                      bukti.buktiPembayaran.split('\\')[2]
                    }`}
                    alt='bukti'
            ...
            <div className='flex justify-center'>
              <button
                className='font-bold uppercase bg-blue-700 hover:bg-blue-800 w-1/2 rounded-full text-2xl py-2 text-center'
                onClick={() => handleSend()}
              >
                telah dikirim
              </button>
            ...
            <ul className='flex flex-col gap-3'>
              <li>
                Tanggal Pengiriman :{' '}
                {moment(pesanan?.tanggalKirim).format('LLL')}
              </li>
